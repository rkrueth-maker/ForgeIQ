...
function normalizeTenantKey(value) {
  ...
  if (key.includes('..')) throw new Error('Tenant key is invalid.');
  return key;
}
...
function compileInstallation(engineConfig, businessPack, options = {}) {
  const engineErrors = validateEngineConfig(engineConfig);
  const packErrors = validateBusinessPack(businessPack);
  ...
  const license = clone(options.license || {
    licenseId: `EVAL-${tenantKey.toUpperCase()}`,
    status: 'ACTIVE',
    type: 'evaluation',
    validUntil: null,
    tenantLimit: 1,
    assignedTenants: [tenantKey],
    allowedTiers: ['Core']
  });
  const licenseErrors = validateLicense(license, tenantKey, tier);
  const errors = [...engineErrors, ...packErrors, ...licenseErrors];
  ...
  const modules = engineConfig.modules.map(module => ({ ...module, enabled: enabled.has(module.id) }));
  ...
  const providers = {};
  for (const slot of businessPack.providers || []) {
    providers[slot.slot] = {
  ...
  return {
    ...
    tenant: {
      key: tenantKey,
      name: options.tenantName || businessPack.theme.brandName,
      namespace: engineConfig.tenant.namespacePattern.replace('{tenantKey}', tenantKey),
      mode: 'isolated'
    },
    ...
    license: {
      ...
      tenantLimit: license.tenantLimit,
      assignedTenants: Array.from(new Set([...(license.assignedTenants || []), tenantKey])),
      allowedTiers: license.allowedTiers
    },
...
function installBusinessOs({ engineConfig, businessPack, license, outputDir, tenantKey, tenantName, tier, releaseChannel, environment = 'production', force = false }) {
  ...
  fs.mkdirSync(path.join(tenantRoot, 'data'), { recursive: true });
  fs.mkdirSync(path.join(tenantRoot, 'logs'), { recursive: true });
  fs.mkdirSync(path.join(tenantRoot, 'private-files'), { recursive: true });
  ...
  const manifest = {
    ...
    engineId: effective.engine.id,
    engineVersion: effective.engine.version,
    businessPack: effective.businessPack,
    tenant: effective.tenant,
    environment: effective.environment,
    releaseChannel: effective.releaseChannel,
    tier: effective.tier,
    externalActionsEnabled: false,
...
function createBackup(installDir, backupFile) {
  ...
  const tenantRoot = path.join(root, namespace);
  const files = {};
  for (const file of fs.readdirSync(path.join(tenantRoot, 'data'))) {
    const rel = path.posix.join(namespace, 'data', file);
    files[rel] = fs.readFileSync(path.join(tenantRoot, 'data', file), 'utf8');
  }
  for (const file of ['proof-log.jsonl', 'error-log.jsonl']) {
    const rel = path.posix.join(namespace, 'logs', file);
    files[rel] = fs.readFileSync(path.join(tenantRoot, 'logs', file), 'utf8');
  }
...
function migrateEffectiveConfig(config, targetSchemaVersion = CURRENT_SCHEMA_VERSION) {
  ...
  while (migrated.schemaVersion < targetSchemaVersion) {
    ...
    throw new Error(`No migration available from schema ${migrated.schemaVersion}.`);
  }
  return migrated;
}
