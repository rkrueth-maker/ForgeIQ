...
function boAiCoach_(payload){
 payload=payload||{};
 const context=boAiSafeContext_(payload.context||{});
 const task=String(payload.task||'Help me use this screen.');
 return boAiChat_({message:'Teach me the next best step for this task: '+task+'. Give only the next step, explain why it matters, and mention any approval required.',context:context});
}
...
function boAiPrepareEmail_(payload){
 payload=payload||{};
 const to=String(payload.to||'').trim(),subject=String(payload.subject||'').trim(),request=String(payload.request||'').trim();
 ...
 const draft={to:to,subject:subject||'Highway 38 follow-up',body:ai.text,createdAt:new Date().toISOString(),createdBy:Session.getActiveUser().getEmail()||'current-user'};
 CacheService.getUserCache().put('H38_AI_SEND_'+token,JSON.stringify(draft),600);
 boAiRecordEvent_({type:'email_draft',module:'email',outcome:'prepared'});
 return{confirmationToken:token,draft:draft,expiresInSeconds:600,requiresConfirmation:true};
}
function boAiSendEmail_(payload){
 payload=payload||{};
 boAssert_(String(payload.confirmation||'').toUpperCase()==='SEND','Explicit SEND confirmation is required.');
 const key='H38_AI_SEND_'+String(payload.confirmationToken||''),cache=CacheService.getUserCache(),raw=cache.get(key);
 boAssert_(raw,'Email confirmation expired or is invalid. Prepare the draft again.');
 const draft=JSON.parse(raw);
 GmailApp.sendEmail(draft.to,draft.subject,draft.body);
 cache.remove(key);
 boAiRecordEvent_({type:'email_send',module:'email',outcome:'confirmed_send'});
 return{sent:true,to:draft.to,subject:draft.subject,sentAt:new Date().toISOString()};
}
function boAiSaveLayout_(layout){
 const clean=boAiSanitizeLayout_(layout||{});
 PropertiesService.getUserProperties().setProperty('H38_AI_LAYOUT',JSON.stringify(clean));
 boAiRecordEvent_({type:'layout_change',module:String(clean.startModule||''),outcome:'saved'});
 return clean;
}
function boAiTelemetry_(event){boAiRecordEvent_(event||{});return{recorded:true};}
function boAiRecommendations_(){
 const events=boAiEvents_(),counts={};
 events.forEach(function(e){const key=[e.type||'event',e.module||'general',e.outcome||''].join('|');counts[key]=(counts[key]||0)+1;});
 const recommendations=[];
 Object.keys(counts).sort(function(a,b){return counts[b]-counts[a];}).slice(0,8).forEach(function(key){const p=key.split('|'),n=counts[key];if(n<3)return;if(p[0]==='module_open')recommendations.push({type:'layout',title:'Pin '+p[1],reason:'Opened '+n+' times recently.',risk:'low',requiresApproval:false});if(p[0]==='workflow_error')recommendations.push({type:'module',title:'Review '+p[1]+' workflow',reason:n+' recent workflow errors.',risk:'medium',requiresApproval:true});if(p[0]==='ai_chat')recommendations.push({type:'coaching',title:'Create guided help for '+p[1],reason:'Users repeatedly asked AI for help here.',risk:'low',requiresApproval:true});});
 return recommendations.slice(0,5);
}
function boAiRecordEvent_(event){
 const props=PropertiesService.getScriptProperties(),events=boAiJson_(props.getProperty('H38_AI_EVENTS'),[]);
 const clean={at:new Date().toISOString(),type:String(event.type||'event').slice(0,40),module:String(event.module||'').slice(0,40),outcome:String(event.outcome||'').slice(0,60),durationMs:Number(event.durationMs)||0,count:Number(event.count)||0,role:String((boGetClientContext()||{}).role||'').slice(0,30)};
 events.push(clean);while(events.length>H38_AI_EVENT_LIMIT)events.shift();props.setProperty('H38_AI_EVENTS',JSON.stringify(events));
}
function boAiEvents_(){return boAiJson_(PropertiesService.getScriptProperties().getProperty('H38_AI_EVENTS'),[]);}
function boAiSafeContext_(context){
 const clean={},allowed=['module','screen','recordType','recordId','recordSummary','task','businessName'];
 allowed.forEach(function(k){if(context[k]!==undefined&&context[k]!==null)clean[k]=String(context[k]).slice(0,k==='recordSummary'?6000:500);});
 return clean;
}
...
function boAiOpenAi_(instructions,input){
 ...
 boAssert_(key,'AI is not configured. Add OPENAI_API_KEY to Apps Script properties.');
 ...
 const code=result.getResponseCode(),raw=result.getContentText();
 boAssert_(code>=200&&code<300,'AI request failed ('+code+').');
 const json=JSON.parse(raw),text=String(json.output_text||boAiExtractText_(json)||'').trim();
 boAssert_(text,'AI returned no answer.');
 return{text:text,durationMs:Date.now()-started};
}
