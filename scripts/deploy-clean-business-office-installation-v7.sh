...
PATCHED="${RUNNER_TEMP:?RUNNER_TEMP is required}/deploy-clean-business-office-installation-v7-runtime.sh"
cp "$SOURCE" "$PATCHED"
...
run_action=r'''run_action() {
  local action="$1" payload_file="$2" raw_file="$3" result_file="$4" access_token status="" attempt
...
NODE
  ...
  for attempt in $(seq 1 24); do
    ...
    if [[ "$attempt" = "24" ]]; then
      echo "HOLD — authenticated acceptance endpoint did not become callable for ${action}; HTTP ${status}." >&2
      head -c 1200 "$raw_file" >&2 || true
      return 1
    fi
    sleep 5
  done
