...
post(){
  local url="$1" body="$2" file="$3" status
  status="$(curl -L --post302 --post303 -sS -o "$file" -w '%{http_code}' -H 'Content-Type: application/json' --data-binary "@$body" "${url}?bo_token=${TOKEN}")"
  printf '%s' "$status" > "${file}.status"
  test "$status" = 200
  node - "$file" <<'NODE'
...
deploy(){
  local dir="$1" title="$2" key="$3" version id
  (cd "$dir" && clasp push --force --json) > "$OUT/${key}-push.json"
  (cd "$dir" && clasp create-version "$title ${GITHUB_SHA}" --json) > "$OUT/${key}-version.json"
  version="$(jval "$OUT/${key}-version.json" versionNumber)"
  test -n "$version"
  (cd "$dir" && clasp create-deployment --versionNumber "$version" --description "$title ${GITHUB_SHA}" --json) > "$OUT/${key}-deployment.json"
...
cp "$APP/appsscript.json" "$APP/appsscript.final.json"
cp "$APP/BusinessOffice_Auth.gs" "$APP/BusinessOffice_Auth.final.gs"
cp "$APP/BusinessOffice_Installer.gs" "$APP/BusinessOffice_Installer.final.gs"
cp "$APP/BusinessOffice_Web.gs" "$APP/BusinessOffice_Web.final.gs"
...
path=root/'BusinessOffice_Web.gs'
text=path.read_text().replace('function doGet()','function boPrivateDoGet_()',1)
path.write_text(text)
PY
manifest "$APP/appsscript.final.json" "$APP/appsscript.json"

python3 - "$APP" "$TOKEN" "$CONFIG_TITLE" <<'PY'
from pathlib import Path
import json,sys
root=Path(sys.argv[1]);token=json.dumps(sys.argv[2]);title=json.dumps(sys.argv[3])
(root/'BusinessOffice_StandaloneHarness.gs').write_text(f'''const BO_STANDALONE_TOKEN_={token};
...
cp "$APP/BusinessOffice_Auth.final.gs" "$APP/BusinessOffice_Auth.gs"
cp "$APP/BusinessOffice_Installer.final.gs" "$APP/BusinessOffice_Installer.gs"
cp "$APP/BusinessOffice_Web.final.gs" "$APP/BusinessOffice_Web.gs"
cp "$APP/appsscript.final.json" "$APP/appsscript.json"
rm -f "$APP/BusinessOffice_Auth.final.gs" "$APP/BusinessOffice_Installer.final.gs" "$APP/BusinessOffice_Web.final.gs" "$APP/appsscript.final.json"
