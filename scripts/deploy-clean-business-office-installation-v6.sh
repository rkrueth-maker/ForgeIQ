...
PATCHED="${RUNNER_TEMP:?RUNNER_TEMP is required}/deploy-clean-business-office-installation-v6-runtime.sh"
cp "$SOURCE" "$PATCHED"
...
new='''  local stderr_file="${raw_file%.json}.stderr.txt" exit_code=0
  set +e
  (cd "$PROJECT" && clasp run boCleanExecute --params "$params" --json) > "$raw_file" 2> "$stderr_file"
  exit_code=$?
  set -e
  if [[ "$exit_code" != "0" ]]; then
