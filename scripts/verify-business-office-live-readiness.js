...
const files={live:'apps-script/business-office/BusinessOffice_ControlPlane_25_LiveReadiness.gs',equipmentCore:'apps-script/business-office/BusinessOffice_ControlPlane_30_Equipment.gs',directLive:'apps-script/business-office/BusinessOffice_ControlPlane_Live_Client.html',directEquipment:'apps-script/business-office/BusinessOffice_Equipment_Client.html',web:'apps-script/business-office/BusinessOffice_Web.gs',clientManifest:'apps-script/business-office/BusinessOffice_ClientManifest.gs',portalBridge:'apps-script/core-engine/owner-portal-next/Portal_ControlPlane.js',portalLive:'apps-script/core-engine/owner-portal-next/Portal_ControlPlane_Live_Client.html',portalEquipment:'apps-script/core-engine/owner-portal-next/Portal_Equipment_Client.html',portalIndex:'apps-script/core-engine/owner-portal-next/Portal_Index.html',raw:'apps-script/core-engine/owner-portal-next/Portal_RawIncludes.js',productionRegistry:'apps-script/business-office/BusinessOffice_ModuleRegistry.gs',reusableRegistry:'apps/business-office/BusinessOffice_ModuleRegistry.gs'};
...
for(const key of ['live','equipmentCore','clientManifest','portalBridge','portalLive','portalEquipment','raw']){try{new vm.Script(read(key),{filename:files[key]});check(`syntax ${files[key]}`,true);}catch(error){check(`syntax ${files[key]}`,false,error.message);}}
...
const live=read('live'),equipment=read('equipmentCore'),web=read('web'),clientManifest=read('clientManifest'),bridge=read('portalBridge'),index=read('portalIndex'),raw=read('raw'),direct=read('directLive')+read('directEquipment'),portal=read('portalLive')+read('portalEquipment');
check('direct Business Office loads live and equipment clients through manifest',web.includes('boRenderClientIncludes_()')&&['BusinessOffice_ControlPlane_Live_Client','BusinessOffice_Equipment_Client'].every(name=>clientManifest.includes(`'${name}'`)));
check('direct live and equipment clients are included once',['BusinessOffice_ControlPlane_Live_Client','BusinessOffice_Equipment_Client'].every(name=>(clientManifest.match(new RegExp(name,'g'))||[]).length===1));
...
const result={status:failures.length?'HOLD':'PASS',generatedAt:new Date().toISOString(),passed:passes.length,failed:failures.length,passes,failures,controlledClientManifest:true};
