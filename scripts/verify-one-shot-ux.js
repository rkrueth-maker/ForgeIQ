...
const required=[
  'ux-unified-public.css','platform-unified.css','platform-states.js','request-flow.js','customer-portal-ux.js','customer-portal-unification.js','index.html','products.html','start-request.html','customer-portal.html','public-expansion.js',
  'supabase/migrations/20260717_customer_portal_quote_project_ux.sql',
  'apps-script/unified-shell/Unified_PublicIntake.gs',
  ...
  'apps-script/core-engine/owner-portal-next/Portal_Product_Unification_Styles.html','apps-script/core-engine/owner-portal-next/Portal_RawIncludes.js','apps-script/core-engine/owner-portal-next/Portal_Index.html'
];
required.forEach(file=>check(`required ${file}`,exists(file)));

['public-expansion.js','platform-states.js','request-flow.js','customer-portal-ux.js','customer-portal-unification.js','customer-portal-supabase.js','apps-script/core-engine/owner-portal-next/Portal_OneShot_Client.html'].forEach(file=>{
...
need('index.html','assets/approved-website-images/10-project-planning-documents.jpg','homepage uses clean approved planning photography');
check('homepage removes embedded mockup and invisible navigation hotspots',!home.includes('assets/approved-homepage-mockup.png')&&!home.includes('class="hotspot')&&!/hs-(?:logo|home|solutions|examples|proof|about|portal|request)/.test(home));
...
check('homepage promotes Quote Builder and Business System',home.includes('Software for small business')&&home.includes('Highway 38 Quote Builder')&&home.includes('Highway 38 Business System'));
...
check('request uses one visible secure final submission action',request.includes('id="request-submit"')&&request.includes('type="submit">Submit Request</button>')&&request.includes('data-intake-endpoint='));
check('email remains hidden fallback only',request.includes('id="email-summary" hidden>Email fallback')&&requestFlow.includes('emailFallback'));
...
check('direct intake returns request confirmation and protects duplicates',requestFlow.includes('submitDirect')&&requestFlow.includes('postMessage')&&requestFlow.includes('Request received')&&publicIntake.includes('DUPLICATE_ACCEPTED')&&publicIntake.includes('Owner Approval Required'));
...
check('customer UX enhancement loaded',customer.includes('customer-portal-ux.js')&&customer.includes('customer-portal-unification.js'));
check('customer portal message language is explicit',customer.includes('Send message to Highway 38 review queue'));
check('customer portal remains noindex',customer.includes('noindex,nofollow'));
const customerUx=read('customer-portal-ux.js');
const customerUnification=read('customer-portal-unification.js');
check('customer onboarding and notifications are present',customerUnification.includes('customerOnboarding')&&customerUnification.includes('customerNotifications'));
...
check('interactive controls meet minimum target',read('visual-cleanup.css').includes('min-height:44px')&&css.includes('min-height:44px')&&read('platform-unified.css').includes('min-height:44px'));
const ownerCss=read('apps-script/core-engine/owner-portal-next/Portal_OneShot_UX_Styles.html');
const unifiedCss=read('apps-script/core-engine/owner-portal-next/Portal_Product_Unification_Styles.html');
...
check('Quote Builder mobile tables become cards',unifiedCss.includes('.quote-builder-shell table')&&unifiedCss.includes('td:before')&&unifiedCss.includes('min-width:0!important'));
